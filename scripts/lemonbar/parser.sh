...
while read -r line; do
  case $line in
    SYS*) # conky
      ...
      # cpu
      cpu_color=$col_main
      if [ "$(echo ${sys_arr[4]} | cut -d% -f1)" -gt 90 ] || # cpu >90%
           [ "$(echo ${sys_arr[5]} | cut -d. -f1)" -gt 10 ]; then # load >10.0
        cpu_color=$col_sec
      fi
      cpu="${sys_arr[4]} ${sys_arr[5]}" # cpu, load
      cpu="%{B${cpu_color}}${sep_left}  %{T2}${cpu}%{T-} ${sep_right}"
      # diskspace
      diskspace="${sys_arr[6]}"
      diskspace="%{B${col_main}}${sep_left}  %{T2}${diskspace}%{T-} ${sep_right}"

      battery_color=$col_main
      if [ "$(battery.sh percent)" -lt 15 ]; then
        battery_color=$col_sec
      fi
      if [ "${sys_arr[7]}" = "G" ]; then # mysterious G dirupts the order
        ...
        downspeed="%{B${battery_color}}${sep_left} ﲐ %{T2}${downspeed}%{T-} ${sep_right}"
      else
        # battery
        battery="${sys_arr[7]} %{T2}${sys_arr[8]}%%{T-}" # icon, percentage
        battery="%{B${battery_color}}${sep_left} ${battery} ${sep_right}"
    ...
    NET*) # wifi/vpn
      ...
      if [ "$wifi_state" -eq 1 ]; then
        if [ "$vpn_state" -eq 1 ]; then
          network="%{B${col_active}}${sep_left} 直 %{T2}vpn%{T-} ${sep_right}"
        else
          if [ "$homenet_state" -eq 1 ]; then
            network="%{B${col_sec}}${sep_left} 直 %{T2}home%{T-} ${sep_right}"
          else
            network="%{B${col_sec}}${sep_left} 直 %{T2}no vpn%{T-} ${sep_right}"
          fi
        fi
      else
