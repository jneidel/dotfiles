...
while read -r line; do
  case "$line" in
    "New tunnel state: Connected"|"device (AirVPN-eu): Activation: successful, device activated."|"device (homewg): Activation: successful, device activated.")
      VPN=1 ;;
    "New tunnel state: Disconnected"|"Error: Failed to set DNS"|"device (AirVPN-eu): state change: "*" -> disconnected"|"device (homewg): state change: "*" -> disconnected")
      VPN=0 ;;
    ...
    "device (ruckersdorf_wg): state change: "*" -> disconnected")
      HOMENET=0 ;;
    "device (ruckersdorf_wg): Activation: successful, device activated.")
      HOMENET=1 ;;
  esac
  echo "NET$WIFI $VPN $HOMENET"
done
